    ...
    internal class HttpPipelineClientFactory : IMsalHttpClientFactory
    {
        ...
        private class PipelineHttpMessageHandler : HttpMessageHandler
        {
            ...
            protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
            {
                ...
                byte[] contentBytes = new byte[pipelineResponse.ContentStream.Length];

                await pipelineResponse.ContentStream.ReadAsync(contentBytes, 0, contentBytes.Length).ConfigureAwait(false);

                string contentStr = Encoding.UTF8.GetString(contentBytes);

                pipelineResponse.ContentStream.Seek(0, System.IO.SeekOrigin.Begin);
