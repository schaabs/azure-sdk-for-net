    ...
    public class VisualStudioCodeCredential : TokenCredential
    {
        ...
        private async ValueTask<AccessToken> GetTokenImplAsync(TokenRequestContext requestContext, bool async, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var result = await _client.AcquireTokenByRefreshToken(requestContext.Scopes, storedCredentials, cloudInstance, tenant, requestContext.ClaimsChallenge, async, cancellationToken).ConfigureAwait(false);
