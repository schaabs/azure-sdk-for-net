...
namespace Azure.Identity
{
    ...
    public class DeviceCodeCredential : TokenCredential
    {
        ...
        private async ValueTask<AccessToken> GetTokenImplAsync(bool async, TokenRequestContext requestContext, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (Record != null)
                {
                    try
                    {
                        AuthenticationResult result = await Client.AcquireTokenSilentAsync(requestContext.Scopes, Record, requestContext.ClaimsChallenge, async, cancellationToken).ConfigureAwait(false);
        ...
        {
            AuthenticationResult result = await Client.AcquireTokenWithDeviceCodeAsync(context.Scopes, code => DeviceCodeCallbackImpl(code, cancellationToken), context.ClaimsChallenge, async, cancellationToken).ConfigureAwait(false);
