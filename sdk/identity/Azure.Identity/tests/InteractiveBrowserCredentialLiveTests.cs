...
namespace Azure.Identity.Tests
{
    public class InteractiveBrowserCredentialLiveTests
    {
        ...
        {
            var cred = new InteractiveBrowserCredential(new InteractiveBrowserCredentialOptions { CacheProvider = DefaultTokenCacheProvider.WithUnencryptedFallback });

            // this should pop browser
            AuthenticationRecord record = await cred.AuthenticateAsync();

            var cred2 = new InteractiveBrowserCredential(new InteractiveBrowserCredentialOptions { CacheProvider = DefaultTokenCacheProvider.WithUnencryptedFallback, AuthenticationRecord = record });
